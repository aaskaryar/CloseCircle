...
class FollowerViewModel: ObservableObject{
    ...
    func updateHobbies(newHobbies : [HobbyModel]){
        ...
        for hobby in oldHobbies{
            ...
            if(!newHobbies.contains(hobby)){ // means we removed this hobby
                ...
                var oldHobbyFollowers : [String] = hobby.followers ?? [String]()
                var newHobbyFollowers : [String] = newHobbies.first(where: {$0.id == hobby.id})!.followers ?? [String]()
        ...
        for newAddedHobby in newAddedHobbies {
            ...
            if let followers = newAddedHobby.followers{
    ...
    func refreshFollowers(size: Int, followersList : [String]){
        ...
        var followersGoingToGet : [String] = followersList
    ...
    func forceUnfollow(uid: String, hobbyId : String){
        ...
        let delete = (self.userInfo.followerUsers[uid]?.count == 1) 
        
        unfollowHobbyInFirestore(myUid: uid, hobbyOwnerUid: userInfo.uid, hobbyId: hobbyId, delete: delete)
    }
}
