...
struct CustomTabBarView_Previews: PreviewProvider {
    static let tabs:[TabBarItem] = [
     ...
     // force unwrap come back and fix
...
extension CustomTabBarView{
        private func tabView(tab: TabBarItem) -> some View {
                VStack {
                    let str = tab.iconName + ".fill"
                    let name = selection != tab ? tab.iconName : str
                    ...
                    Text(tab.title)
                    .font(.system(size:10,weight: .semibold,design:
                            .rounded))
                }
